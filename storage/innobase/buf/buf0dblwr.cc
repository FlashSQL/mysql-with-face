...
flush:
    ...
    if (srv_use_ssd_cache) {
        if (page_num != 0) {
            ...
            if (ssd_cache_size_over) {
                read_buf = read_ssd_cache_for_gsc(first_idx, total_page_num,
                                                    ssd_cache_size_over_first);
            }

            ssd_cache_buf = rebuild_write_buf_for_ssd_cache(first_idx,
                            total_page_num, gsc_page_num,
                            write_buf, read_buf, ssd_cache_size_over_first);
            ...
            for (ulint i = 0; i < page_num; i++) {
                ...
                update_ssd_cache_info((const_cast<buf_page_t*>(&block->page))->space,
                                        ...
                                        meta_idx, first_idx, read_buf, ssd_cache_size_over_first);
                meta_idx++;
            }
                ...
                if (io_fix == BUF_IO_NONE) {
                    ...
                    if (old_entry->flags & BM_VALID) {
                        ...
                        if (ssd_cache_size_over_first) {                        
                            ...
                            if (meta_idx < first_idx) {
                                gc_idx = (ssd_cache_size - first_idx) + meta_idx;
                            } else {
                                gc_idx = meta_idx - first_idx;
                            }

                            memcpy(gc_buf, read_buf + UNIV_PAGE_SIZE * gc_idx,       
                                    UNIV_PAGE_SIZE);                            
                        }                                                       
    ...
    bool    ssd_cache_size_over_first)  /*!< in: true if SSD cache is filled for the first time */
{
    ...
    ssd_cache_writeback(meta_idx, first_idx, read_buf, ssd_cache_size_over_first);
    ...
    for (ulint i = 0;i < total_page_num; i++) {
        ...
        if ((ssd_meta_dir[meta_idx].flags & BM_REF) &&
            ...
            if (first_idx + gsc_idx >= ssd_cache_size) {
                tmp_idx = (first_idx + gsc_idx) - ssd_cache_size;
            } else {
                tmp_idx = first_idx + gsc_idx;
            }

            update_ssd_cache_info(ssd_meta_dir[meta_idx].space, ssd_meta_dir[meta_idx].offset,
                                    ssd_meta_dir[meta_idx].lsn, tmp_idx,
                                    first_idx, read_buf, ssd_cache_size_over_first);

            gsc_idx++;
        } else if ((ssd_meta_dir[meta_idx].flags & BM_REF) &&
                   (ssd_meta_dir[meta_idx].flags & BM_GSC)) {
            if (first_idx + gsc_idx >= ssd_cache_size) {
                tmp_idx = (first_idx + gsc_idx) - ssd_cache_size;
            } else {
            ...
            update_ssd_cache_info(0, 0, 0, tmp_idx,
                                    first_idx, read_buf, ssd_cache_size_over_first);
...
retry:
	...
	if (srv_use_ssd_cache && !zip_size) {
        ...
        ssd_cache_buf = rebuild_write_buf_for_ssd_cache(first_idx,
                        total_page_num, gsc_page_num,
                        ((buf_block_t*) bpage)->frame, read_buf,
                        ssd_cache_size_over_first);
